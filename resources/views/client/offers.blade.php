...
@section('content')
                    ...
                    <div class="row gy-3">
                        ...
                        <div class="col-lg-5 col-12 d-flex justify-content-lg-end align-items-start gap-2">
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu drop-filter p-2">
                                    <h6 class="dropdown-header text-dark px-0 fw-semibold">Sort By</h6>
                                    <li>
                                        <div class="form-check py-2">
                                            <input class="form-check-input sort-filter" type="radio" name="sort"
                                                value="price_asc">
                                            <label class="form-check-label">
                                                Price (lowest first)
                                            </label>
    ...
    <div class="container my-lg-5 my-3">
        <h4 class="py-3 mb-0">Best offers</h4>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('.category-filter').change(function() {
                ...
                $.ajax({
                    url: '{{ route('Client.getSubcategories') }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                    ...
                    success: function(response) {
                        ...
                        $('#subcategory-buttons').append(
                            `<div class="btn filter-button rounded-pill w-auto active" data-subcategory="all"><span>{{ __('trans.all') }}</span></div>`
                        );
                        response.forEach(function(subcategory) {
                            $('#subcategory-buttons').append(`
                        <div class="btn filter-button rounded-pill w-auto" data-subcategory="${subcategory.id}">
                            <span>${subcategory['title_'+lang]}</span>
                        </div>
                    `);
                        });
                    }
            ...
            $('#filter_button').click(function() {
                ...
                $.ajax({
                    url: '{{ route('Client.filterProductsOffers') }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        category_id: category_id,
                    ...
                    success: function(response) {
                        // Update the product list with the response HTML
                        console.log("test filter offers");
                        $('#product_list').empty().html(response.html);

                        // Function to initialize product card
                        function initializeProductCard(card) {
                            let colorButtons = card.querySelectorAll('.color');

                            const defaultButton = Array.from(colorButtons).find(
